...
export default defineConfig(({ mode }) => {
  ...
  return {
    main: {
      plugins: [
        // test-workbench_change: Disable externalizeDepsPlugin for VS Code integration
        // Bundle all dependencies into the output file instead of expecting node_modules at runtime
          ...
          ? [
              viteStaticCopy({
                ...
                targets: [
                  ...
                  // test-workbench_change: Copy native modules for VS Code integration
                  { src: 'node_modules/better-sqlite3', dest: 'node_modules' },
                  { src: 'node_modules/bindings', dest: 'node_modules' },
                  { src: 'node_modules/file-uri-to-path', dest: 'node_modules' },
                ],
              }),
            ]
          : []),
      ...
      build: {
        ...
        // test-workbench_change: Bundle all dependencies for VS Code integration
        ssr: {
          // Prevent Vite from externalizing any packages except native modules
          noExternal: true,
          // Only externalize Node.js built-ins and native modules (they must be external)
          external: ['electron', 'original-fs', 'better-sqlite3', 'sharp', '@whiskeysockets/baileys'],
        },
        rollupOptions: {
          // Don't externalize any dependencies except Node.js built-ins and native modules
          external: (id) => {
            // Only externalize Node.js built-ins, electron, and native modules
            if (id === 'electron' || id === 'original-fs') return true;
            if (id.startsWith('node:')) return true;
            // Externalize native modules that can't be bundled
            if (id === 'better-sqlite3' || id === 'sharp' || id === '@whiskeysockets/baileys') return true;
          ...
          output: {
            // test-workbench_change: Output CommonJS with .cjs extension for VS Code integration
            format: 'cjs',
