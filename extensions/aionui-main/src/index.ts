...
// test-workbench_change: Disable Sentry when running in VS Code integration
// Sentry causes module loading issues in the integrated environment
const isVSCodeIntegration = process.env.VSCODE_AIONUI_INTEGRATION === '1';

if (!isVSCodeIntegration) {
  // Only load Sentry in standalone mode
...
// test-workbench_change: Log single instance lock status for debugging
console.log('[AionUi] ========================================');
console.log('[AionUi] STARTUP - Single instance lock check');
console.log('[AionUi] ========================================');
console.log('[AionUi] isE2ETestMode:', isE2ETestMode);
console.log('[AionUi] AIONUI_E2E_TEST env:', process.env.AIONUI_E2E_TEST);
console.log('[AionUi] deepLinkFromArgv:', deepLinkFromArgv);
console.log('[AionUi] argv:', process.argv);
console.log('[AionUi] cwd:', process.cwd());
console.log('[AionUi] app.isPackaged:', app.isPackaged);
console.log('[AionUi] ========================================');

const gotTheLock = isE2ETestMode ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
console.log('[AionUi] Got the lock:', gotTheLock);

if (!gotTheLock) {
  console.warn('[AionUi] ❌ Another instance is already running; current process will exit.');
  app.quit();
} else {
  console.log('[AionUi] ✅ Single instance lock acquired, continuing startup...');
...
// test-workbench_change: Skip protocol registration when in VS Code integration mode
// because VS Code's app is already ready when we load AionUI
if (!isVSCodeIntegration) {
  ...
  console.log('[AionUi] Skipping protocol.registerSchemesAsPrivileged in VS Code integration mode');
}
...
const handleAppReady = async (): Promise<void> => {
  console.log('[AionUi] ========================================');
  console.log('[AionUi] handleAppReady called');
  console.log('[AionUi] ========================================');
...
console.log('[AionUi] Setting up app.whenReady() handler...');
void app
  .whenReady()
  .then(() => {
    console.log('[AionUi] app.whenReady() fired, calling handleAppReady...');
...
app.on('quit', (_event, exitCode) => {
  console.log('[AionUi] ========================================');
  console.log(`[AionUi] QUIT EVENT (exitCode=${exitCode})`);
  console.log('[AionUi] ========================================');
  // Print stack trace to see where quit was called from
  console.trace('[AionUi] Quit stack trace:');
});

// In this file you can include the rest of your app's specific main process
// code. You can also put them in separate files and import them here.

// Export initializeProcess for VS Code integration
export { initializeProcess };
