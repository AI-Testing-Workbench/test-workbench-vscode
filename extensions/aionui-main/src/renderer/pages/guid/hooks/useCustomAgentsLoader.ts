...
export const useCustomAgentsLoader = ({
  ...
  // test-workbench_change: Provide default preset assistants to avoid empty state
  const [customAgents, setCustomAgents] = useState<AcpBackendConfig[]>(() => {
    // Convert ASSISTANT_PRESETS to AcpBackendConfig format
    return ASSISTANT_PRESETS.map((preset) => ({
      ...
      prompts: preset.promptsI18n?.['en-US'],
      promptsI18n: preset.promptsI18n,
      defaultEnabledSkills: preset.defaultEnabledSkills,
    }));
  });
