...
function buildWin32Setup(arch: string, target: string): task.CallbackTask {
	...
	return (cb) => {
		...
		if (quality === 'stable' || quality === 'insider') {
			// test-workbench_change start - Skip appx package generation
...
function defineWin32CombinedTasks(arch: string, target: string) {
	...
	const vscodeBuildTask = task.task(`vscode-${arch === 'x64' ? 'win32-x64' : 'win32-arm64'}-min`) as task.Task;
	const innoUpdaterTask = task.task(`vscode-win32-${arch}-inno-updater`) as task.Task;
	const setupTask = task.task(`vscode-win32-${arch}-${target}-setup`) as task.Task;
